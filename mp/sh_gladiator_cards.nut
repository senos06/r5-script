...
{
	handle.overrideName = "user"
	handle.overrideCharacter = GetItemFlavorByAsset($"settings/itemflav/character/lifeline.rpak")
	handle.overrideSkin = GetItemFlavorByAsset($"settings/itemflav/character_skin/lifeline/lgnd_v22_goldenweek.rpak")
	//handle.overrideStance = GetItemFlavorByAsset($"settings/itemflav/gcard_stance/pathfinder/heirloom_01.rpak")
	handle.overrideFrame = GetItemFlavorByAsset($"settings/itemflav/gcard_frame/lifeline/epic_v22_goldenweek01.rpak")

	handle.overrideBadgeList[0] = GetItemFlavorByAsset($"settings/itemflav/gcard_badge/account/account_level.rpak")
	handle.overrideBadgeDataIntegerList[0] = 0

	// handle.overrideBadgeList[1] = GetItemFlavorByAsset($"settings/itemflav/gcard_badge/account/account_level.rpak")
	// handle.overrideBadgeDataIntegerList[1] = 0 //dataInteger

	// handle.overrideBadgeList[2] = GetItemFlavorByAsset($"settings/itemflav/gcard_badge/account/account_level.rpak")
	// handle.overrideBadgeDataIntegerList[2] = 0

	handle.overrideTrackerList[0] = GetItemFlavorByAsset($"settings/itemflav/gcard_tracker/lifeline/v23_goldenweek_career_kills.rpak")
	handle.overrideTrackerDataIntegerList[0] = 20

	handle.overrideTrackerList[1] = GetItemFlavorByAsset($"settings/itemflav/gcard_tracker/lifeline/v23_goldenweek_career_wins.rpak")
	handle.overrideTrackerDataIntegerList[1] = 0

	handle.overrideTrackerList[2] = GetItemFlavorByAsset($"settings/itemflav/gcard_tracker/lifeline/v23_goldenweek_career_damage_done.rpak")
	...
	while ( true )
	{
		...
		while ( fileLevel.menuGladCardPreviewCommandQueue.len() > 0 )
		{
			...
			switch( mgcpc.previewType )
			{
				...
				{
					SetNestedGladiatorCardOverrideMeleeSkin( fileLevel.currentMenuGladCardHandle, mgcpc.flavOrNull )
					break
				}
	...
	foreach ( int index, ItemFlavor badge in badgeList )
	{
		...
		AddCallback_EntitiesDidLoad( void function() : ( badgeList ) {
			...
			foreach ( ItemFlavor badge in badgeList )
			{
				...
				Assert( IsValidStatEntryRef( unlockStatRef ), "Badge '" + string(ItemFlavor_GetAsset( badge )) + "' refers to non-existant stat: '" + unlockStatRef + "'" )
				StatEntry unlockStat = GetStatEntryByRef( unlockStatRef )

				if ( dynamicTextStatRef != "" )
				{
					Assert( IsValidStatEntryRef( dynamicTextStatRef ), "Badge '" + string(ItemFlavor_GetAsset( badge )) + "' refers to non-existant stat: '" + dynamicTextStatRef + "'" )
	...
	if ( handle.cardRui != null )
	{
		...
		if ( handle.presentation == eGladCardPresentation.FULL_BOX )
		{
			                                                             ...
			                                                             
			{
				...
				if ( GetCurrentPlaylistVarBool ( "Ranked_ShowRank" , true ) == false )
				{
					showRanked = false
				}
...
int function GetPlayerBadgeDataInteger( EHI playerEHI, ItemFlavor badge, int badgeIndex, ItemFlavor ornull character, bool showOneTierHigherThanIsUnlocked = false )
{
	...
	if ( dynamicTextStatRef != "" )
	{
		...
		dynamicStatVal = GetStat_Int( FromEHI( playerEHI ), dynamicStat, IsLobby() ? eStatGetWhen.CURRENT : eStatGetWhen.START_OF_CURRENT_MATCH )
	}
